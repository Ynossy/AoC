...
def find_path(data, n_fw_min, n_fw_max):
    ...
    distances = [
        [[float("inf")] * 4 * n_fw_max for _ in range(len(data[0]))]
        for _ in range(len(data))
    ]
    distances[0][0] = (
        [0] * 4 * n_fw_max
    )  # every stepcount in every direction in v and h
    while (
        ...
        for ddir in [-1, 0, 1]:
            if ddir == 0 and d >= n_fw_max - 1 or ddir != 0 and 0 <= d < n_fw_min - 1:
                continue
            new_dir = (heading + ddir) % 4
            new_d = d + 1 if ddir == 0 else 0
            state_idx = new_dir * n_fw_max + new_d
...
if __name__ == "__main__":
    ...
    print(f"Part2: {part2(data)}")  # 735 too high, 737 too high, 734 right
