def find_shortest(start, end, heightmap):
    ...
    return path_len[end[0]][end[1]]


def main():
    with open("input.txt") as f:
        heightmap = [
            [ord(i) - ord("a") for i in line] for line in f.read().strip().split("\n")
        ]
    size_x = len(heightmap)
    size_y = len(heightmap[0])
    start = [
    ...
    print(find_shortest(start, end, heightmap))  # 517

    min_path = min([find_shortest(s, end, heightmap) for s in start_a])  # 512
    print(min_path)
